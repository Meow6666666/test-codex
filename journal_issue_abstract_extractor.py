...
def is_valid_issue_url(value: str) -> bool:
...
def main() -> int:
    ...
    parser.add_argument("issue_url", nargs="?", help="Journal issue URL")
    ...
    # Ignore notebook-injected flags such as `-f <kernel.json>`
    # In notebook kernels, argv often contains: -f /path/to/kernel.json
    kernel_arg_mode = "-f" in unknown or (args.issue_url and str(args.issue_url).endswith(".json"))

    if not args.issue_url or (args.issue_url and not is_valid_issue_url(args.issue_url) and kernel_arg_mode):
        print("No valid issue_url provided. Running in notebook/kernel mode; showing help and exiting.")
        parser.print_help()
        return 0

    if not is_valid_issue_url(args.issue_url):
        parser.error("issue_url must be a full URL starting with http:// or https://")
